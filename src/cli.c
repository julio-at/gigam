...
/* escape helper for single-quoted SQL literals */
...
static int cmd_sport(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"create")==0) {
    ...
    static struct option o[]={{"name",1,0,'n'},{0,0,0,0}};
    int ch,ix=0;
    ...
    snprintf(q,sizeof(q),
      ...
      "ON DUPLICATE KEY UPDATE name=VALUES(name)", ne);
...
static int cmd_league(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"create")==0) {
    ...
    if(!sport_id || !name){ fprintf(stderr,"required: --sport-id --name\n"); return 2; }
    char ne[256]; esc_str(c,name,ne,sizeof(ne));
    char q[1024];
    snprintf(q,sizeof(q),
      "INSERT INTO leagues(sport_id,name) VALUES(%ld,'%s') "
      "ON DUPLICATE KEY UPDATE name=VALUES(name)", sport_id, ne);
    if (db_exec(c,q)!=0) { return 5; }
    printf("OK\n"); return 0;
  }
  ...
  if (strcmp(sub,"list")==0) {
    ...
    if (sport_id>0) {
      char q[512];
      snprintf(q,sizeof(q),
        "SELECT id,name,sport_id FROM leagues WHERE sport_id=%ld ORDER BY id", sport_id);
...
static int cmd_team(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"create")==0) {
    ...
    if(!league_id || !name){ fprintf(stderr,"required: --league-id --name\n"); return 2; }
    char ne[256]; esc_str(c,name,ne,sizeof(ne));
    char q[1024];
    snprintf(q,sizeof(q),
      "INSERT INTO teams(league_id,name) VALUES(%ld,'%s') "
      "ON DUPLICATE KEY UPDATE name=VALUES(name)", league_id, ne);
    if (db_exec(c,q)!=0) { return 5; }
    printf("OK\n"); return 0;
  }
  ...
  if (strcmp(sub,"list")==0) {
    ...
    if (league_id>0) {
      char q[512];
      snprintf(q,sizeof(q),
        "SELECT id,name,league_id FROM teams WHERE league_id=%ld ORDER BY id", league_id);
      return exec_and_print(c,q);
    } else {
...
static int cmd_runner(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"list")==0) {
    long bm=0;
    static struct option o[]={{"bookmaker-id",1,0,'b'},{0,0,0,0}};
    int ch,ix=0;
    ...
    char q[1024];
    snprintf(q,sizeof(q),
      "SELECT r.id,r.name,r.is_default,u.username "
      "FROM runners r JOIN users u ON u.id=r.user_id "
      "WHERE r.bookmaker_id=%ld ORDER BY r.id",
      bm);
    return exec_and_print(c,q);
  }
  ...
  if (strcmp(sub,"set-default")==0) {
    long bm=0, rid=0;
    static struct option o[]={{"bookmaker-id",1,0,'b'},{"runner-id",1,0,'r'},{0,0,0,0}};
    int ch,ix=0;
...
static int cmd_bettor(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"list")==0) {
    long runner=0;
    static struct option o[]={{"runner-id",1,0,'r'},{0,0,0,0}};
    int ch,ix=0;
    ...
    char q[512];
    snprintf(q,sizeof(q),
      "SELECT id,code,display_name,created_at FROM bettors WHERE runner_id=%ld ORDER BY id",
      runner);
    return exec_and_print(c,q);
  }
...
static int cmd_event(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"create")==0) {
    ...
    char se[64];
    esc_str(c, starts, se, sizeof(se));

    char q[512];
    snprintf(q,sizeof(q),
      "INSERT INTO events(league_id,starts_at,home_team_id,away_team_id) "
      "VALUES(%ld,'%s',%ld,%ld)",
      league, se, home, away);
  ...
  if (strcmp(sub,"list")==0) {
    return exec_and_print(c,
      "SELECT id,league_id,DATE_FORMAT(starts_at,'%Y-%m-%d %H:%i') AS starts_at,"
...
static int cmd_quote(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"add")==0) {
    ...
    double line=0.0, line_b=0.0, price=0.0, price_b=0.0;
    int asian=0;
    ...
    if (asian && (line_b==0.0 || price_b<=1.0)) {
      fprintf(stderr,"asian requires: --line-b --price-b\n");
      return 2;
    }
    ...
    if (!strcmp(me,"moneyline") || !strcmp(me,"threeway")) {
      snprintf(line_sql,sizeof(line_sql),"NULL");
    } else {
      snprintf(line_sql,sizeof(line_sql),"%.2f", line);
    }

    if (asian) {
      snprintf(lineb_sql,sizeof(lineb_sql),"%.2f", line_b);
      snprintf(priceb_sql,sizeof(priceb_sql),"%.4f", price_b);
    } else {
      snprintf(lineb_sql,sizeof(lineb_sql),"NULL");
      snprintf(priceb_sql,sizeof(priceb_sql),"NULL");
    }
  ...
  if (strcmp(sub,"list")==0) {
    long event=0, bm=0;
    static struct option o[]={{"event-id",1,0,'e'},{"bookmaker-id",1,0,'b'},{0,0,0,0}};
    int ch,ix=0;
    ...
    char q[1024];
    snprintf(q,sizeof(q),
      "SELECT id,market_type,side,COALESCE(CAST(line AS CHAR),'-') AS line,is_asian,"
      "COALESCE(CAST(line_b AS CHAR),'-') AS line_b,price_decimal,"
      "COALESCE(CAST(price_decimal_b AS CHAR),'-') AS price_b,"
      "DATE_FORMAT(captured_at,'%%Y-%%m-%%d %%H:%%i:%%s') AS captured_at "
      "FROM quotes WHERE event_id=%ld AND bookmaker_id=%ld ORDER BY id DESC",
      event, bm);
    return exec_and_print(c,q);
  }
...
static int cmd_bet(int argc, char** argv, MYSQL* c) {
  ...
  if (strcmp(sub,"place")==0) {
    long bm=0,event=0,runner=0,bettor=0;
    const char* market=NULL;
    const char* side=NULL;
    double line=0.0, price=0.0, line_b=0.0, price_b=0.0; /* line_b reserved for future asian split */
    long stake=0;
    int asian=0;
    ...
    (void)line_b; /* silence until we add asian split-stake */
    ...
    /* Try to link to an existing quote id (optional) */
    char qset[1024];
    if(!strcmp(me,"moneyline")||!strcmp(me,"threeway")) {
      snprintf(qset,sizeof(qset),
        "SET @qid := (SELECT id FROM quotes "
        "WHERE event_id=%ld AND bookmaker_id=%ld AND market_type='%s' AND side='%s' "
        "ORDER BY id DESC LIMIT 1);",
        event,bm,me,se);
    } else {
      snprintf(qset,sizeof(qset),
        "SET @qid := (SELECT id FROM quotes "
        "WHERE event_id=%ld AND bookmaker_id=%ld AND market_type='%s' AND side='%s' AND line=%.2f "
        "ORDER BY id DESC LIMIT 1);",
        event,bm,me,se,line);
    }
    if (db_exec(c,qset)!=0) {
  ...
  if (strcmp(sub,"list")==0) {
    ...
    char q[1024];
    snprintf(q,sizeof(q),
      "SELECT b.id AS bet_id, DATE_FORMAT(b.placed_at,'%%Y-%%m-%%d %%H:%%i:%%s') AS placed_at, "
      "b.market_type,b.pick_side,COALESCE(CAST(b.line AS CHAR),'-') AS line,b.is_asian,b.price_decimal,"
      "ROUND(b.stake_cents/100,2) AS stake_usd "
      "FROM bets b WHERE b.bookmaker_id=%ld ORDER BY b.id DESC LIMIT 200",
      bm);
    return exec_and_print(c,q);
  }
